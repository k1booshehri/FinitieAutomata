    ...
    def DrawDFA(self):
        ...
        for a in self.states:
            ...
            if a in self.finalStates:
                net.add_node(s, label=s ,color='#00ff1e')
            else:
                net.add_node(s, label=s,color='#dd4b39')
        for key in self.transitions:
            x='-'.join(key[0])
            y=key[1]
            z='-'.join(key[2])
            prevlabel = ""
            for dict in net.get_edges():
                if dict["from"] == x and dict["to"] == z:
                    prevlabel = dict["label"]
            net.add_edge(x, z, label=y + prevlabel,color='#162347')
        net.show('DFA.html')
    def DrawSDFA(self):
        net = Network(directed=True)
        for a in self.states:
        ...
        for key in self.transitions:
            ...
            for dict in net.get_edges():
                if dict["from"] == x and dict["to"] == z:
                    prevlabel = dict["label"]
            net.add_edge(x, z, label=y + prevlabel)
        net.show('SDFA.html')
...
class NFA():
    ...
    def DrawNFA(self):
        ...
        for a in self.states:
            if a in self.finalStates:
                net.add_node(a, label=a ,color='#00ff1e')
            else:
                net.add_node(a, label=a,color='#dd4b39')
